...
def biomart_query(queries, filter_name, attributes, use_grch37=False):
    """
    Construct an XML query for BioMart, send it, decode the result and return it as an iterator.
...
def uniprot_groups_to_genes(uniprot_groups, databases, use_biomart):
    """
    Maps uniprot ID groups to hgnc gene symbols. Also returns groups that could not be mapped.
    Merges the mappings per group and creates as reverse mapping, from genes to groups.

    :param uniprot_groups: groups of UniProt IDs, as found in a protein dataframe, may contain isoforms and modifications
    ...
    :return: a dict that maps genes to groups, one that maps groups to genes and a list of uniprot ids that were not found
