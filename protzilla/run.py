...
class Run:
    ...
    def create(cls, run_name, workflow_config_name="standard"):
        run_path = Path(f"{PATH_TO_PROJECT}/user_data/runs/{run_name}")
    ...
    def continue_existing(cls, run_name):
        with open(
            f"{PATH_TO_PROJECT}/user_data/runs/{run_name}/run_config.json", "r"
        ) as f:
    ...
    def __init__(self, run_name, workflow_config_name):
        ...
        with open(
            f"{PATH_TO_PROJECT}/user_data/workflows/{workflow_config_name}.json", "r"
        ) as f:
